...
- You MUST work on the feature branch (`feature/<slug>`) — NEVER on develop/development
...
2. **Verify** you are on the correct feature branch:
   ...
   ```
   If not, prompt user to switch: "Please switch to branch `feature/<slug>`"
...
```bash
opencode run -m <model> "Implement TASK-N for the <service> service based on the service PRD at <path>. Feature branch: feature/<slug>."
```
